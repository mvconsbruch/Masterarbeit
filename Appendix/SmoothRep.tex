...
\begin{defi}[Locally Profinite Group]
  A \emph{locally profinite group} is a topological group $G$ such that every open neighborhood of the identity in $G$ contains a compact open
  subgroup of $G$.
\end{defi}
...
\begin{defi}[Smooth and Admissible Representations]\label{def:SmoothAndAdmRep}
  A representation $\pi\colon G \to \GL(V) \in \Rep G$ is called \emph{smooth}
  ...
  A smooth representation $(\pi, V)$ is \emph{admissible} if it is smooth and for every
...
\begin{defi}[Algebraic Induction]
  Let $G$ be any group and let $H$ be a subgroup of $G$. We define the \emph{algebraic
...
\begin{rmk} 
  ...
  left-$G$-action on $\algInd_H^G(V)$. This action is precisely the one described above.
\end{rmk}
...
\begin{defi}[Smooth Induction]\label{def:SmoothInduction}
  We define the \emph{smooth induction functor} 
...
\begin{defi}[Compact Induction]\label{def:CompactInduction}
  Let $\pi\colon H \to \GL(V)$ be a smooth representation of $H$. Then we define $\cInd_H^G(\pi)$, the \emph{compactly induced} representation of $\pi$, as the subrepresentation of $\Ind_H^G(\pi)$ with underlying vector
  space
  \begin{equation*}
...
\end{defi}
Note that if $H$ is cocompact in $G$, we have $\cInd_H^G = \Ind_H^G$. 
...
\begin{proof}
  ...
  \end{equation*}
  Here the first isomorphism is by Tensor-Hom-adjunction. The second equality uses that $V$ is a smooth representation of $G$.
\end{proof}
\end{thm}
...
\begin{proof}
  ...
  isomorphism. Now the claim follows by taking smooth parts and applying 
  \cref{lem:SmoothRestriction}.
\end{proof}
\end{lem}
\begin{rmk} 

...
unanswered, two of which we aim to address in the present text. First, 
Harris--Taylor prove the local Langlands correspondence
as a pure existence theorem, shedding no light on the question of an explicit
description of the correspondence. Secondly, and this point is of rather
conceptual nature, their proof makes essential use of global arguments,
ultimately requiring to realize $E$ as the completion of some global field at
one of its places.
...
\begin{thm*}[Main Results of this Thesis]
  \leavevmode \begin{enumerate}
    ...
    \item Let $\pi$ be a depth zero supercuspidal representation of $\GL_n(E)$.
      Admitting the results of non-Abelian Lubin--Tate theory, one can
      calculate $\rec_E(\pi)$ and $\JL(\pi)$ explicitly only using the geometry
      of the Lubin--Tate tower.
  \end{enumerate}
\end{thm*}
The first part is an infinite level analogue of the construction by Yoshida,
and second part is an application of Mieda's ideas.
For precise statements, see \cref{prop:SpecialFiberOfAffinoidIsLusztig}
and \cref{thm:MainRes1} below. 

The structure of the thesis is as follows.

\textbf{ 1} reviews the theory of formal $\cO_E$-modules, which
...
In \textbf{s 2 and 3}, we review classical Lubin--Tate theory as in
...
\textbf{ 4} introduces the Lubin--Tate space at infinite level. 
...
\textbf{ 5} reviews the results of Mieda's paper \cite{mieda2016geometric}.
We do not go into details, but hope to provide some insight into his methods. 

\textbf{ 6} deals with Deligne--Lusztig theory for $\GL_n(\FF_q)$. 
...
Finally, in \textbf{ 7}, we collect the results of the 
...
\textbf{Fields.} Throughout the text, we fix a non-Archimedian local field $E$

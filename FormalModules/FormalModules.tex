...
Let $(s_1, \dots, s_n) \in \Tilde H(S)^n$, and write $x_i = \lambda(s_i) \in \Nilp^\flat(S)$,
which are elements in $S^\cici$ with distinguished $p$-power roots.
Here $\lambda: \Tilde H \to \Nilp^\flat$ is the isomorphism from Section
...
where 
\begin{itemize}
  \item the sum ranges over $n$-tuples $(a_1, \dots, a_n)$ of integers (including negative
    integers) satisfying $a_1 + \dots + a_n = n (n-1)/2$, subject to the
...
\begin{proof}
  ...
  \begin{multline*}
    ...
    = \det \left( \sum_{m \in \Z} \frac{x_i^{q^{mn + j}}}{\pi^m} \right)_{
      1 \leq i \leq n,
      0 \leq j \leq n-1},
  \end{multline*}

...
Quillen \cite[Appendix A]{keller1990chain}. We introduce the extensions and 
rigidified extensions, 
\begin{align*}
...
\begin{defi}[Exact Category]
  ...
  of some $(i,d) \in E$, second components are called deflation.
...
\begin{proof}
  ...
  $\beta'(\bX, \bY) = \bY$. 
  The morphism of $A$-module laws $f^*\cE \to \cE$ is given by 
...
\begin{proof}
  ...
  \end{gather*}
  The morphism $\cE \to g_*\cE$ is given by $(\bX, \bZ) \mapsto (g(\bX), \bZ)$. 
  One may check that these data glue to a formal $A$-module
...
\begin{defi}[Extension]
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  commute. We denote the set of isomorphism classes of extensions of $\cF$ by
  $\cF'$ as $\Ext(\cF, \cF')$. 
\end{defi}
...
\begin{prop}
  ...
  \end{equation*}
  Explicitely, this bijection is given by seding $\bgg$ to the morphism
  $i_\bgg \in \Hom_{\FMLArbOver A R}(E, E')$, where 
  $i_\bgg (\bX, T) = (\bX +_{F'} \bgg(T), T)$. In particular, there is a bijection
  \begin{equation*}
    \Ext(\cF, \cF') \cong \frac{\SymCoc^2(F, F')}{\{\delta \bgg 
    \mid \bgg \in (R\llbr \bZ \rrbr)^m \text{ and } \bgg(0) = 0\}}.
  \end{equation*}
...
\begin{defi}[Quasi-Logarithms]\label{def:QuasiLogarithm}
  ...
  of the power series appearing in $\delta g$ (cf. \ref{def:SymCoc1})
...
\begin{defi}[Formal Module associated to $R$-module]
  Suppose that $A$ is any ring, $R$ is an $A$-algebra and $M$ is a finite
  projective $R$-module. Then we write $\GG_a \otimes M$ for the additive
  formal module associated to $M$. As a formal scheme, this formal module is given by
  \begin{equation*}
    \GG_a \otimes_A M \cong \spf R \llbr M^\vee \rrbr,
  \end{equation*}
  where $R \llbr M^\vee \rrbr$ denotes the completion of $\Sym_R(M^\vee)$ with respect
  to the ideal generated by $M^\vee$. The $A$-module structure is the canonical
  additive one.
  Note that $\Lie(\GG_a \otimes M) = M$ by design.
\end{defi}
We specialize to the situation where $A$ is a complete discrete valuation ring
...
Let $H \in \FMLOver AR$ be the standard formal $A$-module of height $n$ over $R$. 
...
% subsubsection The Universal Additive Extension (end)

% subsection Explicit Dieudonne Theory (end)

...
\end{equation}
for some power series $g(T) = c_1 T + c_2 T^2 + \dots \in k_R\llbr T \rrbr$ with 
$c_1 \in k_R^\times$. 
...
\end{equation*}
We write $U_{f,0}$ for the base change of $U_f$ to $k_R$, that is
\begin{equation*}
  U_{f,0}: \Adm k_R \to \Set, \quad S \mapsto \left\{(x_0,x_1, \dots) \in
                              \prod_\N S^\cici \mid f(x_{i+1}) = x_i\right\}.
\end{equation*}
...
All of the results in this section rest ultimately on the following simple observation.
\begin{lem}\label{lem:cryscalc}
  For any two elements $s_1, s_2 \in S$ with $s_1 \equiv s_2$ mod $J$, we have 
...
\begin{cor}
  ...
  with $s_1 \equiv s_2 \mod J$
  such that $f(s_1)$ and $f(s_2)$ exist (for example if $f$ is a polynomial
  or $s_1, s_2 \in S^\cici$), we have 
  \begin{equation*}
    f^k(s_1) \equiv f^k(s_2) \pmod {J^{k+1}}.
  \end{equation*}
\begin{proof}
  The proof follows the same strategy as above. Given $s_1 - s_2 \in J^k$, we find
  \begin{equation*}
    f(s_1) - f(s_2) \in g(s_1^{q})-g(s_2^{q}) + J( h(s_1) - h(s_2) )
  \end{equation*}
  for certain $g,h \in R\llbr T \rrbr$. The claim follows, as $h(s_1) - h(s_2) \in J^k$ 
  and 
\end{proof}
\end{cor}

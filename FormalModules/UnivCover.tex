...
\begin{rmk} 
  In case where $H$ comes from a Lubin--Tate $\cO_K$-module law of height $n$,
  the bijections
  \begin{equation*}
...
\begin{lem}\label{lem:LogInTermsOfNil}
  ...
  \end{equation*}
\begin{proof}
  This follows directly from the above remark. Let $x \in \Tilde H(S)$ and write
  $(y, y^{1/q}, \dots)$ for $\lambda(x)$. We have $x_0 = \lim_{s \to \infty}
  [\pi^s]_H(y^{-ns})$, hence
  \begin{equation*}
    \log_H(x_0) = \lim_{s \to \infty} \pi^s \log_H(y^{1/q^{ns}}) = 
    \lim_{s \to \infty} \sum_{i=0}^\infty \frac{y^{q^{n(i-s)}}}{\pi^{i-s}}
    = \sum_{i=-\infty}^\infty y^{q^{ni}}{\pi^i}.
  \end{equation*}
\end{proof}
\end{lem}

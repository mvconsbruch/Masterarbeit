...
We define a formal model $\cX = \spf C_n$ as follows. Let $H$ be the standard
...
\begin{lem}\label{lem:PropertiesOfTau}
  ...
  \begin{equation*}
    \tau \equiv \det(\alpha_i^{q^j}) \xi^{1 + q + \dots + q^{n-1}} \pmod {\text{what?}}
  \end{equation*}
  holds.
\begin{proof}
  We have $t \neq 0$ and $[\varpi]_H(t) = 0$, so the first two assertions follow just 
  as above, with the minor modification that $\wedge H$ isn't the standard formal
  $\cO_E$-module but has the slightly different logarithm 
  $\log_{\wedge H}(T) = \sum_{i=0}^\infty (-1)^{i(n-1)} \frac{T^{q^n}}{\varpi^n}$.
  The congruence is a corollary of the approximation of 
  $\Delta$ in Lemma \ref{lem:DeltaApprox}.
\end{proof}
\end{lem}

We now define the formal model $\cX$. We abbreviate with $(x_i^{q^{-m}})_{m \in
\N_0}$ the system of 
$q$-th power roots $X_i^{q^{-m}}/\xi^{q^{-m}}$ of elements in $\cO_C \llbr
X_1^{q^{-\infty}}, \dots, X_n^{q^{-\infty}}\rrbr$, and define systems of $q$-th
power roots $\Delta'$ and $\tau'$ as
\begin{equation*}
  \begin{array}c
  \Delta'(x_1, \dots, x_n)^{q^{-m}} \coloneq (\xi^{q^{-m}})^{-(1 + q + \dots +
  q^{n-1})} \Delta(X_1, \dots, X_n) \in \cO_C \llbr x_1^{q^{-\infty}}, \dots
  , x_n^{q^{-\infty}}\rrbr  \\
  \text{and} \\
...
\begin{proof}
  ...
  \ref{lem:DeltaApprox} \red{todo: make more precise}. The statement about $\tau'$
  follows after calculating its absolute value (for any choice of
  non-archimedian norm on $\C_p$).
\end{proof}
\end{lem}
...
\end{prop}
\begin{rmk} 
  Disregarding finiteness conditions, $\cX$ can be interpreted as a subspace of the
  formal admissible blowing up of $\cM_{\infty, \cO_\Cp}^{(0)}$ along the closed

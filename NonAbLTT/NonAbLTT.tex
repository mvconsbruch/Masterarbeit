...
Akin to the action of $D^\times$, the idea is to put 
...
\end{equation*}
Now, given a tuple $(H, \iota, \phi) \in \cM_m(R)$, we put
$$(H, \iota, \phi).g = (H', \iota', \phi'),$$
where 
...
\end{equation*}
This construction is independent of the choice of $r$. \todo{why?}
...
generic fiber of $\cM_K \in \FSchOver {\cO_{\br F}}$\todo{reference
(Berthelot?)}. Fix an algebraic closure $\C_p$ of $F$ and a prime number $\ell
...
By functoriality of the generic fiber functor, the right action of $D^\times \times
\GL_n(F)$ on the tower $\{\cM_K\}_{K \subset \GL_n(\cO_F)}$ yields a left-action
of $D^\times \times \GL_n(F)$ on $\HLT$. 

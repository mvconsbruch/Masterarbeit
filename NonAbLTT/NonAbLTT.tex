...
$\cO_E$-module $H$ of height $n$ (cf. \cref{sub:Hazewinkels FuncEq and the
...
\begin{equation*}
  ...
  \iota\colon \XX \to H \otimes_R \Fqbar \text{ a quasi-isogeny}\}/\sim,
\end{equation*}
...
Note that forgetting the level structure induces a map 
$\cM_m(R) \to \cM_0(R)$, which yields a stratification 
$\cM_m = \coprod_{j \in \Z} \cM_m^{(j)}$. The functors
$\cM_m^{(j)}$ are, for varying $j \in \Z$, pairwise isomorphic.
Furthermore, for non-negative integers $m' \leq m$, we have natural morphisms
$\cM_m \to \cM_{m'}$ by restricting the level structure. Our main interest will
...
if $g \in \GL_n(\cO_F)$. To extend this action to all of $\GL_n(F)$, we

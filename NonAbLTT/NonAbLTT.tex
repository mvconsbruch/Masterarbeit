...
Let $\sigma \in \Gal(\br E / E)$ be the automorphism corresponding to the
$q$-th power Frobenius automorphism of the residue field of $E$. 
Given any presheaf $\cG \colon \FSchOver {\Spf(\cO_\br E)}^\op \to \Set$, we denote by
$\sigma^* \cG$ the fiber product $\cG \times_{\spf(\cO_\br E), \sigma}
\spf(\cO_\br E)$. In this situation, we have the notion of Weil descent data
(cf. \cite[Definition 3.45]{rapoport1996period}).

\begin{defi}[Weil Descent Datum]\label{def:WeilDescentDatum}
  A Weil Descent Datum for $\cG$ is an isomorphism of functors
  \begin{equation*}
    \alpha \colon \cG \xto\sim \sigma^* \cG.
  \end{equation*}
\end{defi}

Given a Weil descent datum $\alpha$ on $\cG$ and an element $w \in \Weil_E$
such that $w|_{\br E} = \sigma^m$ for $m \in \Z$, we have isomorphisms
\begin{multline}\label{eq:ActionFromWeilDD}
  \cG \times_{\spf(\cO_\br E)} \spf(\cO_{\C_p}) \xto{(1, w)} 
  \cG \times_{\spf(\cO_\br E), \sigma^m} \spf(\cO_{\C_p}) \\
  = \sigma^{m, *} \cG \times_{\spf(\cO_\br E)} \spf(\cO_{\C_p})
  \xto {\alpha^{-m}} \cG \times_{\spf(\cO_\br E)} \spf(\cO_{\C_p}).
\end{multline}
Here, $\alpha^{-m}$ denotes the inverse of the isomorphism $\cG \to 
\sigma^{m, *} \cG$ obtained by iterating the isomorphism $\alpha$. This

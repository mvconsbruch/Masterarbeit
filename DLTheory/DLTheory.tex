...
$G^F$. These virtual representations arise from the $\ell$-adic cohomology (with
...
\begin{defi}[Geometrically Conjugates and Regular Position]\label{def:DLGeomConjRegularPos}
  ...
  of $T^F(k)$. We say that $\theta$ is in general position if it is not fixed by any
...
of $\DL_n$. We set $G = \GL_n$, and we denote by $B^\std$ the standard Borel (of
upper triangular matrices), by $T^\std$ the standard torus (of diagonal matrices)
and by $U^\std$ the standard unipotent matrix (of upper triangular matrices with
...
This yields an isomorphism of representations $R_{\theta \circ \ad h} \cong
R^\theta_{T\subset B}$, where $h$ is chosen as above. If $\theta$ is a
...
\begin{thm}[Deligne--Lusztig Correspondence]\label{thm:DLCorrespondenceForUs}
  Let $\theta$ be a regular character of $\FF_{q^n}^\times$. 
  \begin{enumerate}
    \item We have
      \begin{equation*}
    ...
    \item The natural map 
      \begin{equation*}
        \hH^{n-1}_c(\DL_n, \bar \Q_l)_\theta \to \hH^{n-1}(\DL_n, \bar \Q_l)_\theta
      \end{equation*}
      is an isomorphism.
  \end{enumerate}
\end{thm}

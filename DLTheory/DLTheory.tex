...
\begin{prop}
  The subgroup $n\Z \subset \Z$ acts on $\hH_\DL$ through the character $\gamma:
  n\Z \to \Q^\times$, given by
  $$\gamma(nm) =  (-1)^{(n-1)m} q^{m \frac{n(n-1)}2}.$$
  That is, $\Frob_q^n$ induces multiplication by the scalar
  $(-1)^{n-1}q^{\frac{n(n-1)}2}$ on $\hH_\DL$.
  \begin{proof}
...
$\GL_n(\FF_q) \times \FF_{q^n}^\times \times \Z$-representation 
$\hH_\DL$.
...
\begin{thm}[Structure of $\hH_\DL$]\label{thm:MainThmHDlStructure}
  ...
  \end{equation*}
  Here, $R_\theta$ is a irreducible cuspidal representation, and 

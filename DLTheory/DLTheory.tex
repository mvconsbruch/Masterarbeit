...
X_w$, where $\TT_w^\Frob$ is the $k$-group scheme with $S$-rational points
given by 
\begin{equation*}
...
\begin{proof}
  ...
  \cap_{\mu \in V_0^\vee}D^+(\mu)(S)$. As both sides are Zariski-sheaves, the
  morhpism $\Psi$ is well-defined.
\end{proof}
\end{prop}
...
\begin{lem}\label{lem:ActionsOnDLn}
  The action of $\GL_n(\FF_q)$ on $\DL_n$ is given by
  \begin{equation*}
...
\begin{thm}[Some Results of Deligne-Lusztig Theory]\label{thm:DLTheoryGeneralResults}
  ...
  \begin{enumerate}
      \item \textnormal{\cite[Corollary 1.22]{delignelusztig1976}} Let $Z \subset G$
        ...
        X_{T \subset B}, \bar \Q_l)_{\theta}$ through $\theta|_{Z^F}$.
...
\begin{equation*}
  R^\theta_w = \sum_i (-1)^i \hH_c^i(\DL_n, \bar \Q_l)_\theta.
\end{equation*}
...
\begin{thm}[Deligne--Lusztig Correspondence]\label{thm:DLCorrespondenceForUs}
  ...
  \begin{enumerate}
    \item If $\theta$ is a regular character of $\FF_{q^n}^\times$, we have
      ...
      The representation $R_\theta$ is irreducible and cuspidal, and it's central
...
\begin{prop}
  ...
  $(-1)^{n-1}q^{\frac{n(n-1)}2}$ on $\hH_{\DL,\theta}$.
  \begin{proof}

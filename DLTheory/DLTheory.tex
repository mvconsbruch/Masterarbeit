...
is an application of a more general theory developed by Deligne--Lusztig. 
...
certain characters $\theta$ of $T^F$ a virtual representation $R_{T,\theta}$ of 
$G^F$. These virtual representations arise from the $l$-adic cohomology (with
...
\begin{defi}[Flag Variety]\label{def:FlagVariety}
  ...
  \begin{multline*}
    X(S) = \\ \left\{\cF_1 \subset \cF_2 \subset \dots \subset \cF_{n-1} \subset
      ...
      \text{For all $i$, $\cF_i$ is locally a direct summand} \\
...
$$\Flag = (F_1 \subsetneq F_2 \subsetneq \dots F_{n-1} \subsetneq V) \in X(k).$$
  ...
  \end{equation*}
  where $\Grass_{V,i}$ denotes the Grassmannian with 
  respect to the finite free $\cO_{\spec k}$-module $\tilde V^\vee$, as defined
  in \cite{Ricolfi2022}.
...
\begin{proof}
  ...
  contained in $\cF_{i+1}$ is a polynomial condition.
...
algebraic group $G$. 
It is an elementary fact that $B_{\Flag}$ is a Borel subgroup of $\GL_V$,
...
\begin{proof}
  ...
  there is a Zariski-cover $\phi: S' \to S$ (with structure map to $k$ denoted
...
\textbf{Remark. }The proof shows that the quotient $\GL_V/B$ is trivialized 
Zariski-locally (as opposed to fppf-locally).
...
$U_{\Flag, e_\bullet} \subset \GL_V$ be the (unipotent) isotropy subgroup of
the corresponding element $(\Flag, e_\bullet) \in Y(k)$ under the action of
$\GL_V$. 
...
\begin{prop}[Bruhat Decomposition]\label{prop:BruhatDecomp}
  ...
  Coxeter-length of $w$.
\begin{proof}
   By construction, $O_w$ is locally closed and smooth (orbits are locally
   closed by \cite[Proposition 1.65 b)]{milne2017algebraic}). The remaining claims
...
$w \in \Sigma_n$ if it lies inside $O_w(S)$. 
...
$(w,t) \in \Sigma_n \times (k^\times)^n$ if it appears in $\tilde
O_{w,t}$.
...
\end{equation*}
Note that $X$ and $Y$ are defined over $\FF_q$, and these automorphisms come
from the respective (relative) frobenii of $X$ and $Y$ over $k$. We write
...
Zariski-torsor for an affine group scheme $\TT_w^\Frob \times_k X_w$,
where $\TT_w^\Frob$ is the $k$-group scheme with $S$-rational points given by
\begin{equation*}
...
lines in $V$ that do not lie in the hyperplane spanned by $\mu$.
...
\begin{proof}
  ...
  $\cap_{\mu} D^+(\mu)$, which is the case if any only if $F_1(s) = F_1
  \otimes_{\cO_{\spec k}} \cO_{\spec \kappa(s)}(\{s\})$  does not lie inside
  ...
  given by
  \begin{equation*}
    \Psi: \bigcap_{\mu \in V_0^\vee}D^+(\mu)(S) \to X_w(S), \quad L \mapsto 
  ...
  Hence for any $s \in S$ there is some open affine $\spec R \subset S$ trivializing
  $\cL$. Write $L$ for the corresponding free rank $1$ direct summand of 
  $R^n$. Then $L = \langle v \rangle$ for some $v = (v_1, \dots, v_n) \in R^n$. Now 
...
The (left-)action of $\GL_n(\FF_q) \times \FF_{q^n}^\times$ on $\DL_n$ has the following
explicit description.

\begin{lem}\label{lem:ActionsOnDLn}
  The action of $\GL_n$ on $\DL_n$ is given by
  \begin{equation*}
    (a_{ij})_{i,j}.(x_1, \dots, x_n) = \left(\sum_{j = 1}^n
    a_{ij}x_j\right)_{i = 1, \dots, n}
  \end{equation*}
  The action of $\FF_{q^n}^\times$ is given by
  \begin{equation*}
    d.(x_1, \dots, x_n) = (d^{-1}x_1, \dots, d^{-1}x_n).
  \end{equation*}
\end{lem}

Through the relative Frobenius, we may also define an action of 
$\Z$ on $\DL_n$, sending $1$ to the automorphism given by $x_i \mapsto
\Frob(x_i) = x_i^q$. By construction, the actions of $\GL_n(\FF_q)$, 
$\FF_{q^n}^\times$ and $\Z$ commute.

Note that $g \in \GL_n(\FF_q)$ induces a morphism of schemes
$Y_b \to Y_{b \det(g)}$. Similarly, $\zeta \in \FF_{q^\times}$ 
restricts to $Y_b \to Y_{b \Norm(\zeta)}$, where $\Norm = \Norm_{\FF_{q^n}/\FF_q}$
denotes the norm map of the extension $\FF_{q^n}/\FF_q$. The relative Frobenius
morphism $\DL_n \to \DL_n$ stabilizes the components $Y_b$. In particular, 
the stabilizer subgroup of $\GL_n \times \FF_{q^n}^\times \times \Z$
for each component $Y_b$ is given by
\begin{equation*}
  (\GL_n(\FF_q) \times \FF_{q^n}^\times)^1 \times \Z 
  \coloneq \{(g,d,n) \in \GL_n(\FF_q) \times \FF_{q^n}^\times \times \Z 
  \mid \det(g) \Norm(d) = 1\}.
\end{equation*}
...
\end{equation*}
By the above, this is a representation of $\GL_n(\FF_q) \times
\FF_{q^n}^\times \times \Z$. The following subsection is concerned with the
study of this representation.
...
\subset G$, Deligne and Lusztig construct varieties with right $G^F$-actions
$X_{T\subset B}$ and $\tilde X_{T \subset B}$, constituting a $G^F$-equivariant
Galois-covering
\begin{equation*}
  \tilde X_{T \subset B} \to X_{T \subset B}
\end{equation*}
with Galois-group $T^F$. See \eqref{eq:ExplicitDLVarieties} for explicit
descriptions of these spaces. The space $\tilde X_{T \subset B}$ comes with
commuting actions of $G^F$ and $T^F$, and for characters $\theta$ of $T^F(k)$,
...
\begin{equation} \label{eq:DLRepresentation}
  ...
  \Q_l)_\theta \in \cR(T^F(k) \times G^F(k)).
\end{equation}
Here, the subscript $\theta$ denotes the direct summand of $\hH^i_c(\tilde X_{T
\subset B}, \bar \Q_l)$ where $T^F(k)$ acts by $\theta$.
These representations do only depend on the choice of the torus, not on the
choice of the Borel sugroup
(\cite[Corollary 4.3]{delignelusztig1976}). 

Recall that for a finite group $H$, the Grothendieck group $\cR(H)$ of finite 
dimensional $H$-representations in $\bar \Q_l$ comes with a natural inner product. Indeed,
as $\rho(h)^{\# H} = 1$, any representation takes values in the maximal cyclotomic
...
$r \in \N$. 
The inner product is now defined on representations $\rho, \rho' \in \Rep H$ as
\begin{equation*}
  \langle \rho, \rho' \rangle = \frac 1 {\# H} \sum_{h \in H} \Tr(\rho(h)) \bar {\Tr (\rho'(h))} \in \bar \Q_l.
\end{equation*}
This definition extends linearly to $\cR(H)$. An orthogonal basis for $\cR(H)
\otimes \Q$ is given by the irreducible finite-dimensional representations of
$H$. If $\rho$ is an irreducible representation of $H$ and 
$R$ is an element in $\cR(H) \otimes \Q$, we say that $\rho$ occurs in $R$ if 
$\langle \rho, R\rangle \neq 0$.

\begin{defi}[Geometrically Conjugates and Regular Position]\label{def:DLGeomConjRegularPos}
  Let $T$ be a Frobenius-stable maximal  torus of $G$ and let $\theta$ be a character
  of $T^F(k)$. 
  \begin{enumerate}
    \item We say that $\theta$ is in general position if it is not fixed by any
      non-trivial element of $\Weyl_G(T)^F$. Here $\Weyl_G(T)$ denotes the Weyl
      group $(\Norm_G(T)/T)$.
  \item If $(T', \theta')$ is another pair consisting of a Frobenius-stable maximal
    torus and a character of $T^F(k)$, we say that the pairs $(T,\theta)$ and 
    $(T', \theta')$ are geometrically conjugate if there exists an integer $n$
    such that the pairs $(T, \theta\circ {N_n})$, $(T', \theta' \circ N_n)$ 
    are conjugate, where $N_n$ is the norm function $T^{\Frob^n}(k) \to T^{\Frob}(k)$. 
  \end{enumerate}
\end{defi}

We summarize a few of the results of the theory.

\begin{thm}[Some Results of Deligne-Lusztig Theory]\label{thm:DLTheoryGeneralResults}
  \leavevmode
  \begin{enumerate}
      \item \textnormal{\cite[Corollary 7.7]{delignelusztig1976}} For any
        irreducible representation
          $\rho$ of $G^F$, there exists an $F$-stable maximal torus $T$ and a character
        $\theta$ of $T^F$ such that $\langle \rho, R_T^\theta\rangle \neq 0$.
      \item \textnormal{\cite[Corollary 6.3]{delignelusztig1976}} If 
        $(T, \theta)$ and $(T', \theta')$ are not geometrically conjugate, 
        no irreducible representation of $G^F(k)$ occurs in both
        $R^\theta_T$ and $R^{\theta'}_{T'}$.
      ...
      \item \textnormal{\cite[Corollary 8.3]{delignelusztig1976}} If furthermore
        ...
        one of $\pm R^\theta_T$ is a cuspidal representation of $G^F(k)$.
      \item \textnormal{\cite[Corollary 9.9]{delignelusztig1976}} If furthermore
        ...
        $\Frob(B)$ (cf. the Bruhat decomposition for $G$),
...
We next explain how these results apply to the comactly supported \'etale cohomology
of $\DL_n$. Obviously we set $G = \GL_n$, and we fix the standard
torus $T^\std$ (of diagonal matrices), the standard Borel $B^\std$ (of upper
diagonal matrices) and the standard unipotent subgroup $U^\std$. 

Choose a flag $\Flag \in X_w(k)$, where $w$ is, as usual, $(1\ 2\ \dots \ n)
\in \Sigma_n \cong \Weyl_G(T^\std)$ (although the discussion remains valid for any choice
of permutations). 
Let $B \subset \GL_n$ denote the isotropy subgroup of $\Flag$, and let 
$T \subset B$ be a Frobenius-stable torus in $B$.
...
\begin{equation} \label{eq:ExplicitDLVarieties}
  \begin{array}c
  X_{T \subset B} = \{g \in \GL_n \mid g^{-1} F(g) \in F(U)\}/(T^F(U \cap F(U)))\\
  \text{and} \\
  \tilde X_{T \subset B} = \{g \in \GL_n \mid g^{-1} F(g) \in F(U)\}(U \cap F(U)).
  \end{array}
\end{equation}
...
(i.e., $g.(\Flag^\std,e_\bullet^\std) = (\Flag',e'_\bullet)$). Also, we may
choose $h \in \GL_n(k)$ such that $h(T^\std, B^\std)h^{-1} = (T,B)$. Then
by \cite[Proposition 1.19]{delignelusztig1976}, the map
\begin{equation*}
...
\begin{equation*}
  R^\theta_w = \sum_i (-1)^i \hH_c^i(\DL_n, \bar \Q_l).
\end{equation*}
We may embed the torus $\TT_w$, which is the standard torus with 
rational structure such that $\Frob$ acts by $\ad w \circ \Frob$, into
$\GL_n$. Now it is conjugated to $T^\std$, and this
identification yields isomorphisms $R^{\theta \circ \ad h}_w \cong R^\theta_{T\subset
B}$, where $h$ is chosen as above. If $\theta$ is a character of $\TT_w^\Frob(k)
\cong \FF_{q^n}^\times$, the pair $(T, w \circ \ad h)$ is in regular position
if and only if $\theta$ is regular, in the following sense.
...
\begin{thm}[Deligne--Lusztig Correspondence]\label{thm:DLCorrespondenceForUs}
  \leavevmode
  \begin{enumerate}
    \item If $\theta$ is a regular character of $\FF_{q^n}^\times$, we have
      \begin{equation*}
        \hH_{\DL,\theta} = \hH_c^{n-1}(\DL_n, \bar \Q_l)_\theta = R_\theta \boxtimes
        \theta
      \end{equation*}
      as representations of $\GL_n(\FF_q) \times \FF_{q^n}^\times$.
      The representation $R_\theta$ is irreducible and cuspidal.
    \item The natural map 
      \begin{equation*}
        \hH^{n-1}_c(\DL_n, \bar \Q_l) \to \hH^{n-1}(\DL_n, \bar \Q_l)
      \end{equation*}
      is an isomorphism.
  \end{enumerate}
\end{thm}
...
on $\hH_\DL$. Recall that the Frobenius-automorphism yields an action of 
$\Z$ on $\hH_\DL$. This action admits the following partial description.

\begin{prop}
  The action of $n\Z$ on $\hH_\DL$, induced by the $n$-th power of the
  relative Frobenius automorphism $\Frob_q^n: \DL_n \to \DL_n$ is given by 
  $$\gamma(nm) : x \mapsto (-1)^{(n-1)m} q^{m \frac{n(n-1)}2} x.$$
  That is, $\Frob_q^n$ induces multiplication by the scalar $(-1)^{n-1}q^{\frac{n(n-1)}2}$
  on $\hH_\DL$.
  \begin{proof}
    The result is due to Digne--Michel, cf. \cite{digne1985fonctions}. Also see 
    the proof of \cite[Théorème 3.1.12]{wang2014espace}.
  \end{proof}
\end{prop}

Concludingly, we obtain

\begin{thm}[Structure of $\hH_\DL$]\label{thm:MainThmHDlStructure}
  For a regular character $\theta: \FF_{q^n}^\times \to \bar \Q_l^\times$, 
  $\hH_{\DL, \theta}$ is, as a representation of 
  $\GL_n(\FF_q) \times \FF_{q^n}^\times \times n\Z$, given by 
  \begin{equation*}
